...
void ROSPixelCloudFusionApp::ImageCallback(const sensor_msgs::Image::ConstPtr &in_image_msg)
{
  ...
  image_frame_id_ = camera_info_frame_;// in_image_msg->header.frame_id;//camera_info_frame_;//
  image_size_.height = current_frame_.rows;
  image_size_.width = current_frame_.cols;
}

//hw_start
//bool ROSPixelCloudFusionApp::IsPointInImage(const sensor_msgs::PointCloud2)

void ROSPixelCloudFusionApp::CloudCallback(const sensor_msgs::PointCloud2::ConstPtr &in_cloud_msg)
{
   cloud_msg_ = in_cloud_msg;
   if(!cloud_callback_ok) cloud_callback_ok = true; 
}

void ROSPixelCloudFusionApp::VisionDetectionsCallback(const autoware_msgs::DetectedObjectArray::ConstPtr &in_vision_detections)
{
  if(!cloud_callback_ok) return;
  ...
  pcl::fromROSMsg(*cloud_msg_, *in_cloud);
  ...
  jsk_recognition_msgs::BoundingBoxArray out_box_arr;
  out_box_arr.header = cloud_msg_->header;

  #pragma omp for
    for(size_t i = 0; i < in_vision_detections->objects.size(); i++)
    {
      jsk_recognition_msgs::BoundingBox out_box;
      out_box.header = cloud_msg_->header;
      double pose_x = 0.0;
      double pose_y = 0.0;
      double pose_z = 0.0;
      double length = 0.2;
      ...
      double height = 0.5;
      int cnt = 0;
      if(in_vision_detections->objects[i].label!="WARNING_TRIANGLE") continue;
      for (int row = in_vision_detections->objects[i].y; row < in_vision_detections->objects[i].y+in_vision_detections->objects[i].height; row++)
      {
        for (int col = in_vision_detections->objects[i].x; col < in_vision_detections->objects[i].x+in_vision_detections->objects[i].width; col++)
        {
          std::unordered_map<cv::Point, pcl::PointXYZ>::const_iterator iterator_3d_2d;
          pcl::PointXYZ corresponding_3d_point;
          pcl::PointXYZRGB colored_3d_point;
          iterator_3d_2d = projection_map.find(cv::Point(col, row));//원래 col, row
          if (iterator_3d_2d != projection_map.end())
          {
            corresponding_3d_point = iterator_3d_2d->second;
            cv::Vec3b rgb_pixel = current_frame_.at<cv::Vec3b>(row, col);//원래 row, col
            ...
            if(colored_3d_point.z <= -2.05||colored_3d_point.z > -1.65) continue;
            pose_x += colored_3d_point.x;
            pose_y += colored_3d_point.y;
            pose_z += colored_3d_point.z;
            cnt++;
      ...
      if(cnt != 0)
      {
        out_box.pose.position.x = pose_x / cnt;
        out_box.pose.position.y = pose_y / cnt;
        out_box.pose.position.z = pose_z / cnt;
        out_box.dimensions.x = length;
        out_box.dimensions.y = width;
        out_box.dimensions.z = height;
        out_box_arr.boxes.push_back(out_box);
      }
    }
    ...
    cloud_msg.header = cloud_msg_->header;
    pixel_cloud_box_publisher_.publish(out_box_arr);
...
{
  camera_info_frame_ = in_message.header.frame_id;
...
{
  //get params
  std::string points_src, image_src, detected_objects_vision, camera_info_src, fused_topic_str = "/points_fused";
  std::string name_space_str = ros::this_node::getNamespace();

  ROS_INFO("[%s] This node requires: Registered TF(Lidar-Camera), CameraInfo, Image, and PointCloud.", __APP_NAME__);
  in_private_handle.param<std::string>("points_src", points_src, "/pointcloud/os1");
  ...
  detections_vision_subscriber_ = in_private_handle.subscribe(detected_objects_vision,
                                                               1,
                                                               &ROSPixelCloudFusionApp::VisionDetectionsCallback, this);
  //hw_end
  ...
  pixel_cloud_box_publisher_ = node_handle_.advertise<jsk_recognition_msgs::BoundingBoxArray>("/pixel_cloud_box", 1);
